...
//			console.log("getRealTimeData==========="+responseText)
...
Beans.extendClass('KBO.components.ViewPort', 'Beans.components.UiComponent', {
	...
	_construct: function() {
		...
		EventBus.register(me, 'completedInitializing', function() {
			console.log("ViewPort completedInitializing");
			me.show();
			me.videoListPanel.show();
		});

		EventBus.register(me, 'occuredErrorOnInitializng', function () {
			console.log("ViewPort occuredErrorOnInitializng");
			me.show();
			me.videoListPanel.show();
		}, 1);
...
Beans.extendClass('KBO.components.VideoListPanel', 'Beans.components.UiComponent', {
    ...
    _construct: function() {
        ...
        EventBus.register(me, 'completedInitializing', function() {
        	console.log("VideoListPanel completedInitializing");
...
Beans.extendClass('KBO.components.Popup', 'Beans.components.UiComponent', {
    ...
    _initializeElement: function() {
        ...
        EventBus.register(me, 'occuredErrorOnInitializng', function () {
//        	console.log("occuredErrorOnInitializng======================"+me.is4chAppForReturned);
...
//        	console.log("occuredErrorOnConnectingWSocket======================"+me.is4chAppForReturned);
        ...
        EventBus.register(me, 'occuredErrorOnOTSSignal', function () {
        	me.is4chAppForReturned = false;
        	me._showNoticePopup("위성신호가 미약하여 5채널 서비스가 종료됩니다 <br/>잠시 후 다시 이용해 주세요");
        });
    },
   ...
   _showClosingPopup: function () {
    ...
    _showPopup: function(jsonText) {
        ...
        if (json && json.API.RESULT === 'FALSE') {
			clearInterval(RealtimeData.interval);
            me.isVisibility = true;
            me.setText(me.titleElement, '오늘 경기가 모두 종료되어<br/> 프로모션 채널로 이동합니다.');
            /*
             *  임시로 주석 처리 이후 주석 해제해야함
             */
            me.is4chAppForReturned = false;
            me.show();
        }
    ...
    pressedEnterKey: function(keyCode) {
        var me = this;
        console.log("pressedEnterKey======================"+me.is4chAppForReturned);
        me.hide();
...
//				console.log('==================initializeKeySet==================');
				...
				console.log("initializeKeySet Complete");
			...
			initilaize = function() {
				...
				initializeKeySet();
				console.log("STBService completedInitializing");
...
Beans.defineClass('KBO.app.DataLoader', {
    ...
    loadWebSocketUrl: function (callback) {
        ...
        console.log(AppConfig.mashUpApi.url + '/wsocketInfo?type=TEST&'+DataLoader.getCommonQueryString());
        return Http.get({
...
//        	url: "todayGameList.js"
    ...
    getRealTimeData: function(callback) {
    	console.log(AppConfig.mashUpApi.url + '/realtimeData?' + DataLoader.getCommonQueryString());
        return Http.get({
...
//    	console.log("game 상태======>"+PlayDataManager.channelMap[PlayDataManager.currentChannelNumber].STT_ID);
...
Beans.defineClass('KBO.app.ChannelTuner', {
	...
	_construct: function() {
		...
		EventBus.register(me, 'completedInitializing', function(channels) {
			console.log("ChannelTuner completedInitializing");
			me.createObject();
			var tempLimit = 'none';
...
//			me.getCurrentPlayData();
		...
		EventBus.register(me, 'NoOTSSignal', function() {
			...
			ownerApp.onApplicationDestroyRequest = function() {
				...
				console.log('called NoOTSSignal onApplicationDestroyRequest');
...
//			document.querySelector('#alarm2').style.display = 'block';
	...
	requestRealtimeDate: function() {
		...
		DataLoader.getRealTimeData().then(function(responseText) {
			var json = JSON.parse(responseText);
			try {
			// console.log('retryCountRealTime : ' + me.retryCountRealTime);
...
//			me.getCurrentPlayData();
	...
	requestInningData: function() {		
		...
		if(PlayDataManager.getCurrentGameId() !== undefined) {
			console.log("전체 이닝 정보 가져오기")
			DataLoader.getInningData().then(function(responseText) {
	//			 console.log("requestInningData :"+responseText);
				try {
			...
			me.inningDataJson = null;
			me.reset();
			me.getCurrentPlayData();
		}
//		DataLoader.getInningData(function(json) {
//			
////			var json = JSON.parse(responseText);
////			// console.log('retryCount : ' + me.retryCountInning);
//			if (me.retryCountInning > 2) {
//				me.retryCountInning = 0;
//				EventBus.fire('notRecievedPKGList');
//			}
//			
//			if (json !== '') {
//				PlayDataManager.inningDataJson = json;
//				me.retryCountInning = 0;
//			} else {
//				me.retryCountInning++;
//			}			
...
//		});
	...
	getChannelData: function() {
		...
		// }
		var gSttLData;
		//onGoingGame이 아닌 RealTimeData의 G_R_L 정보로 전체 게임 종료 판단
		var isAirR = "TRUE";
		...
		if(channelsJson != null) {
			for (key in returnList) { 				
				...
				// console.log("gSttLData.STT_ID : "+gSttLData.STT_ID);
				item.STT_ID = gSttLData.STT_ID;
				me.channelMap['' + returnList[key]] = item;
				
				console.log("노출 여부 : "+item.AIR_R)
				if(item.AIR_R == "0") {
					isAirR = "FALSE";
				}
...
//		me.scoreAway = 0;
	...
	setChannelMapData : function(){
		...
		for (key in channelsJson) {
			...
			gSttLData = me.getPlayDataByGameId(item.GAME_ID,channelSTTJson);
//			 console.log("key============>",key+":"+gSttLData.STT_ID);
			 console.log('item.GAME_ID ================================== ' + item.GAME_ID);
			item.STT_ID = gSttLData.STT_ID;
			me.channelMap['' + key] = item;
			if(item.AIR_R == "0") {
				console.log("게임 비노출 : "+item.STT_ID+":"+item.GAME_ID);
				isAirR = "FALSE";
			}
		}
//onGoingGame이 아닌 RealTimeData의 G_R_L 정보로 전체 게임 종료 판단		
	...
	getPlayDataByGameId:function(gameId,jsonArrData){
		...
		for (idx in jsonArrData) {			
			if(gameId == jsonArrData[idx].GAME_ID) {
				// console.log('STT_ID : ' + jsonArrData[idx].STT_ID);
...
//					console.log("subGame =========>"+subGame);
					if (subGame != null) {
						...
						console.log("게임아이디 : "+subChGameId)
						console.log("방문팀 : "+subItem.away)
						console.log("홈팀 : "+subItem.home)
...
//					console.log("item.HPNAME :"+item.HPNAME);
					break;
				}
			}
		}
		...
		if (state === 0 || state === 1) {
			...
			console.log("taggiaVisitData :"+taggiaVisitData);
			if(pitcherVisitData) {
				me.playerMap['' + pitcherVisitData.PY_ID] = 'V';
				me.playerMap['' + pitcherVisitData.PY_ID + 'V'] = pitcherVisitData;
				console.log("pitcherVisitData.PY_ID :"+pitcherVisitData.PY_ID);
...
//					 console.log('tempAway =====================1'+game.BT);
...
//				if (pitcherVs != null) {
		...
		console.log("me.inningNum===============>"+me.inningNum);	
		// score panel 임시정보
//		var tempHinning = [],
//			tempAinning = [];
				...
				if(me.allInning === null ||  me.allInning.length == 0) {
					...
					for (i = 0; i < me.allInning.length; i++) {
						...
						if (item.H_V == 1) { // home
							...
							// console.log("item.INN :"+item.INN+"-"+item.S);
...
Beans.defineClass('KBO.app.RealtimeData', {
    ...
    init: function() {
        ...
        return new Promise(function(resolve, reject) {
        	console.log('ws://' + me.url + '/said=' + STBService.getSAID())
...
//                 console.log('WS Response: ' + e.data);
            ...
            me.ws.onclose = function(e) {
                ...
                if (!me.initialized) {
                	EventBus.fire('occuredErrorOnConnectingWSocket');
                    return;
                }
                setTimeout(function() {
